...
local SR_DebugMenu = Class(Widget,function (self,colors)
    Widget._ctor(self, "SR_DebugMenu")

    self.colors = colors or default_colors
    self:DoInit()
end)

function SR_DebugMenu:DoInit()
...
function SR_DebugMenu:Search(str)
...
function SR_DebugMenu:GetCurrentData()
    return self.stack and #self.stack>0 and self.stack[#self.stack].data or nil
end

function SR_DebugMenu:ShowTop(pos,keys)
    if not (self.stack and #self.stack > 0 ) then return end
    local source = self.stack[#self.stack]
...
function SR_DebugMenu:Push(tb,keys)
